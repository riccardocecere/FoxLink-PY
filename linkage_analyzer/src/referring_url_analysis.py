...
def calculate_all_cluster_labels(clusters):
    labeled_cluster = []
    if (len(list(clusters)) > 0):
        referring_list = generate_referring_lists(clusters)
        referers = calculate_referers(referring_list)
        labeled_cluster = label_cluster_on_referring(referers)
